...
export default function ChatPage() {
  ...
  }, [messages, streamingContent, settings.autoScroll, highlightMessageId]);
  ...
  const sendMessage = async () => {
    ...
    try {
      const controller = api.sendMessageStream(convo.id, userMsg.content, {
        onChunk: (chunk, isMock) => {
          accumulated += chunk;
          setStreamingContent(accumulated);
          if (isMock) setDemoMode(true);
        },
        onDone: (isMock) => {
          if (isMock) setDemoMode(true);
          const finalContent = accumulated || '（回复被中断）';
        ...
        onError: (err) => {
          ...
          const finalContent = accumulated || '抱歉，出了点问题，请重试。';
          setMessages(prev => [...prev, {
            id: Date.now() + 1,
            role: 'assistant',
            content: finalContent,
